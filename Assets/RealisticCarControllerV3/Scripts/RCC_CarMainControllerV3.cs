...
/// </summary>
public class RCC_CarMainControllerV3 : RCC_CoreMain {
	...
	public RCC_WheelColliderController FrontLeftWheelCollider;
	public RCC_WheelColliderController FrontRightWheelCollider;
	public RCC_WheelColliderController RearLeftWheelCollider;
	public RCC_WheelColliderController RearRightWheelCollider;
	public RCC_WheelColliderController[] ExtraRearWheelsCollider;		// Extra Wheels. In case of if your vehicle has extra wheels.

	public RCC_WheelColliderController[] allWheelColliders;		// All Wheel Colliders.
	...
	public bool runEngineAtAwake{get{return RCCSettings.runEngineAtAwakeFlag;}}		    // Engine running at Awake?
	...
	public bool autoReverse{get{if(!externalController) return RCCSettings.autoReverseFlag; else return true;}}                            // Enables / Disables auto reversing when player press brake button. Useful for if you are making parking style game.
	public bool automaticGear{get{if(!externalController) return RCCSettings.useAutomaticGearFlag; else return true;}}                // Enables / Disables automatic gear shifting.
	internal bool semiAutomaticGear = false;			// Enables / Disables semi-automatic gear shifting.
	public bool UseAutomaticClutch { get { return RCCSettingsInstance.useAutomaticClutchFlag; } }
	#endregion
	...
	private AudioClip[] gearShiftingClips{get{return RCCSettings.gearShiftingClipsMass;}}
	private AudioSource crashSound;
	private AudioClip[] crashClips{get{return RCCSettings.crashClipsMass;}}
	private AudioSource reversingSound;
	private AudioClip reversingClip{get{return RCCSettings.reversingClipValue;}}
	private AudioSource windSound;
	private AudioClip windClip{get{return RCCSettings.windClipValue;}}
	private AudioSource brakeSound;
	private AudioClip brakeClip{get{return RCCSettings.brakeClipValue;}}
	private AudioSource NOSSound;
	private AudioClip NOSClip{get{return RCCSettings.NOSClipValue;}}
	private AudioSource turboSound;
	private AudioClip turboClip{get{return RCCSettings.turboClipValue;}}
	private AudioSource blowSound;
	private AudioClip[] blowClip{get{return RCCSettings.blowoutClipMass;}}
	...
	public GameObject contactSparkle{get{return RCCSettings.contactParticlesObject;}}		// Contact Particles for collisions. It must be Particle System.
	...
	/// </summary>
	public delegate void onRCCPlayerSpawned(RCC_CarMainControllerV3 RCC);
	public static event onRCCPlayerSpawned OnRCCPlayerSpawned;

	/// <summary>
	/// On RCC player vehicle destroyed.
	/// </summary>
	public delegate void onRCCPlayerDestroyed(RCC_CarMainControllerV3 RCC);
	public static event onRCCPlayerDestroyed OnRCCPlayerDestroyed;

	/// <summary>
	/// On RCC player vehicle collision.
	/// </summary>
	public delegate void onRCCPlayerCollision(RCC_CarMainControllerV3 RCC, Collision collision);
	public static event onRCCPlayerCollision OnRCCPlayerCollision;
    #endregion

    public RCC_TruckTrailerController attachedTrailer;

	void Awake (){
		
		// Overriding Fixed TimeStep.
		if(RCCSettings.overrideFixedTimeStepFlag)
			Time.fixedDeltaTime = RCCSettings.fixedTimeStepValue;

		// Getting Rigidbody and settings.
		rigid = GetComponent<Rigidbody>();
		rigid.maxAngularVelocity = RCCSettings.maxAngularVelocityValue;
		...
		FrontLeftWheelCollider.wheelModelTransform = FrontLeftWheelTransform;
		FrontRightWheelCollider.wheelModelTransform = FrontRightWheelTransform;
		RearLeftWheelCollider.wheelModelTransform = RearLeftWheelTransform;
		RearRightWheelCollider.wheelModelTransform = RearRightWheelTransform;

		// If vehicle has extra rear wheels, assign them too.
		for (int i = 0; i < ExtraRearWheelsCollider.Length; i++) 
			ExtraRearWheelsCollider[i].wheelModelTransform = ExtraRearWheelsTransform[i];
    ...
    private void CreateAudios(){
        ...
        switch (audioType){
			...
			engineSoundHigh = CreateNewAudioSource(RCCSettings.audioMixerValue, gameObject, engineSoundPosition, "Engine Sound High AudioSource", 5, 50, 0, engineClipHigh, true, true, false);

                if (autoCreateEngineOffSounds){

				engineSoundHighOff = CreateNewAudioSource(RCCSettings.audioMixerValue, gameObject, engineSoundPosition, "Engine Sound High Off AudioSource", 5, 50, 0, engineClipHigh, true, true, false);

                    AddNewLowPassFilter(engineSoundHighOff, 3000f);

                }else{

				engineSoundHighOff = CreateNewAudioSource(RCCSettings.audioMixerValue, gameObject, engineSoundPosition, "Engine Sound High Off AudioSource", 5, 50, 0, engineClipHighOff, true, true, false);
			...
			engineSoundHigh = CreateNewAudioSource(RCCSettings.audioMixerValue, gameObject, engineSoundPosition, "Engine Sound High AudioSource", 5, 50, 0, engineClipHigh, true, true, false);
			engineSoundLow = CreateNewAudioSource(RCCSettings.audioMixerValue, gameObject, engineSoundPosition, "Engine Sound Low AudioSource", 5, 25, 0, engineClipLow, true, true, false);

                if (autoCreateEngineOffSounds){
				
				engineSoundHighOff = CreateNewAudioSource(RCCSettings.audioMixerValue, gameObject, engineSoundPosition, "Engine Sound High Off AudioSource", 5, 50, 0, engineClipHigh, true, true, false);
				engineSoundLowOff = CreateNewAudioSource(RCCSettings.audioMixerValue, gameObject, engineSoundPosition, "Engine Sound Low Off AudioSource", 5, 25, 0, engineClipLow, true, true, false);

                    AddNewLowPassFilter(engineSoundHighOff, 3000f);
                    AddNewLowPassFilter(engineSoundLowOff, 3000f);

                }else{

				engineSoundHighOff = CreateNewAudioSource(RCCSettings.audioMixerValue, gameObject, engineSoundPosition, "Engine Sound High Off AudioSource", 5, 50, 0, engineClipHighOff, true, true, false);
				engineSoundLowOff = CreateNewAudioSource(RCCSettings.audioMixerValue, gameObject, engineSoundPosition, "Engine Sound Low Off AudioSource", 5, 25, 0, engineClipLowOff, true, true, false);
			...
			engineSoundHigh = CreateNewAudioSource(RCCSettings.audioMixerValue, gameObject, engineSoundPosition, "Engine Sound High AudioSource", 5, 50, 0, engineClipHigh, true, true, false);
			engineSoundMed = CreateNewAudioSource(RCCSettings.audioMixerValue, gameObject, engineSoundPosition, "Engine Sound Medium AudioSource", 5, 50, 0, engineClipMed, true, true, false);
			engineSoundLow = CreateNewAudioSource(RCCSettings.audioMixerValue, gameObject, engineSoundPosition, "Engine Sound Low AudioSource", 5, 25, 0, engineClipLow, true, true, false);

                if (autoCreateEngineOffSounds){

				engineSoundHighOff = CreateNewAudioSource(RCCSettings.audioMixerValue, gameObject, engineSoundPosition, "Engine Sound High Off AudioSource", 5, 50, 0, engineClipHigh, true, true, false);
				engineSoundMedOff = CreateNewAudioSource(RCCSettings.audioMixerValue, gameObject, engineSoundPosition, "Engine Sound Medium Off AudioSource", 5, 50, 0, engineClipMed, true, true, false);
				engineSoundLowOff = CreateNewAudioSource(RCCSettings.audioMixerValue, gameObject, engineSoundPosition, "Engine Sound Low Off AudioSource", 5, 25, 0, engineClipLow, true, true, false);

                    if (engineSoundHighOff)
                        AddNewLowPassFilter(engineSoundHighOff, 3000f);
                    if (engineSoundMedOff)
                        AddNewLowPassFilter(engineSoundMedOff, 3000f);
				...
				engineSoundHighOff = CreateNewAudioSource(RCCSettings.audioMixerValue, gameObject, engineSoundPosition, "Engine Sound High Off AudioSource", 5, 50, 0, engineClipHighOff, true, true, false);
				engineSoundMedOff = CreateNewAudioSource(RCCSettings.audioMixerValue, gameObject, engineSoundPosition, "Engine Sound Medium Off AudioSource", 5, 50, 0, engineClipMedOff, true, true, false);
				engineSoundLowOff = CreateNewAudioSource(RCCSettings.audioMixerValue, gameObject, engineSoundPosition, "Engine Sound Low Off AudioSource", 5, 25, 0, engineClipLowOff, true, true, false);
		...
		engineSoundIdle = CreateNewAudioSource(RCCSettings.audioMixerValue, gameObject, engineSoundPosition, "Engine Sound Idle AudioSource", 5, 25, 0, engineClipIdle, true, true, false);
		reversingSound = CreateNewAudioSource(RCCSettings.audioMixerValue, gameObject, gearSoundPosition, "Reverse Sound AudioSource", 10, 50, 0, reversingClip, true, false, false);
		windSound = CreateNewAudioSource(RCCSettings.audioMixerValue, gameObject, windSoundPosition, "Wind Sound AudioSource", 1, 10, 0, windClip, true, true, false);
		brakeSound = CreateNewAudioSource(RCCSettings.audioMixerValue, gameObject, "Brake Sound AudioSource", 1, 10, 0, brakeClip, true, true, false);

        if (useNOS)
			NOSSound = CreateNewAudioSource(RCCSettings.audioMixerValue, gameObject, exhaustSoundPosition, "NOS Sound AudioSource", 5, 10, .5f, NOSClip, true, false, false);
        if (useNOS || useTurbo)
			blowSound = CreateNewAudioSource(RCCSettings.audioMixerValue, gameObject, exhaustSoundPosition, "NOS Blow", 1f, 10f, .5f, null, false, false, false);
        if (useTurbo){
			turboSound = CreateNewAudioSource(RCCSettings.audioMixerValue, gameObject, turboSoundPosition, "Turbo Sound AudioSource", .1f, .5f, 0f, turboClip, true, true, false);
            AddNewHighPassFilter(turboSound, 10000f, 10);
        }
	...
	private void CheckBehavior (){
		...
		if (RCCSettings.selectedBehaviorTypeR == null)
			return;
	...
	public IEnumerator StartEngineDelayed (){
		...
		if (!engineRunning) {
			...
			engineStartSound = CreateNewAudioSource(RCCSettings.audioMixerValue, gameObject, engineSoundPosition, "Engine Start AudioSource", 1, 10, 1, engineStartClip, false, true, true);
	...
	public void Repair(){
		...
		if (!repaired && repairNow){
            ...
            for (int i = 0; i < allWheelColliders.Length; i++){
				...
				allWheelColliders[i].damagedCamberValue = Mathf.Lerp(allWheelColliders[i].damagedCamberValue, 0f, Time.deltaTime * 2f);
				allWheelColliders[i].damagedCasterValue = Mathf.Lerp(allWheelColliders[i].damagedCasterValue, 0f, Time.deltaTime * 2f);
				allWheelColliders[i].damagedToeValue = Mathf.Lerp(allWheelColliders[i].damagedToeValue, 0f, Time.deltaTime * 2f);

				if (Mathf.Abs(allWheelColliders[i].damagedCamberValue) > .05f || Mathf.Abs(allWheelColliders[i].damagedToeValue) > .05f || Mathf.Abs(allWheelColliders[i].damagedCasterValue) > .05f){

					repaired = false;

                }else{

					allWheelColliders[i].damagedCamberValue = 0f;
					allWheelColliders[i].damagedCasterValue = 0f;
					allWheelColliders[i].damagedToeValue = 0f;
	...
	private void Inputs() {
		...
		if (canControl) {
			...
			if (!externalController) {
				...
				inputs = RCC_InputMainManager.GetInputsData();

				if (!automaticGear || semiAutomaticGear) {
					if (!changingGear && !cutGas)
						throttleInput = inputs.throttleInputValue;
					else
						throttleInput = 0f;
				} else {
					if (!changingGear && !cutGas)
						throttleInput = (direction == 1 ? Mathf.Clamp01(inputs.throttleInputValue) : Mathf.Clamp01(inputs.brakeInputValue));
					else
						throttleInput = 0f;
				}

				if (!automaticGear || semiAutomaticGear) {
					brakeInput = Mathf.Clamp01(inputs.brakeInputValue);
				} else {
					if (!cutGas)
						brakeInput = (direction == 1 ? Mathf.Clamp01(inputs.brakeInputValue) : Mathf.Clamp01(inputs.throttleInputValue));
					else
						brakeInput = 0f;
				}

				steerInput = inputs.steerInputValue;
				boostInput = inputs.boostInputValue;
				handbrakeInput = inputs.handbrakeInputValue;

				if(!UseAutomaticClutch)
					clutchInput = inputs.clutchInputValue;
	...
	private void GetExternalInputs(){
		...
		switch(RCCSettings.selectedControllerTypeR){

		case RCC_SettingsData.ControllerType.Keyboard:
			
			if(RCC_InputMainManager.GetKeyDownFlag(RCCSettings.lowBeamHeadlightsKBCode))
				lowBeamHeadLightsOn = !lowBeamHeadLightsOn;

			if(RCC_InputMainManager.GetKeyDownFlag(RCCSettings.highBeamHeadlightsKBCode))
				highBeamHeadLightsOn = true;
			else if(RCC_InputMainManager.GetKeyUpFlag(RCCSettings.highBeamHeadlightsKBCode))
				highBeamHeadLightsOn = false;

			if(RCC_InputMainManager.GetKeyDownFlag(RCCSettings.startEngineKBCode))
				KillOrStartEngine();

			if(RCC_InputMainManager.GetKeyDownFlag(RCCSettings.trailerAttachDetachCode))
				DetachTrailer();

			if(RCC_InputMainManager.GetKeyDownFlag(RCCSettings.rightIndicatorKBCode)){
			...
			if(RCC_InputMainManager.GetKeyDownFlag(RCCSettings.leftIndicatorKBCode)){
			...
			if(RCC_InputMainManager.GetKeyDownFlag(RCCSettings.hazardIndicatorKBCode)){
			...
			if (RCC_InputMainManager.GetKeyDownFlag (RCCSettings.NGearCode))
				NGear = true;

			if (RCC_InputMainManager.GetKeyUpFlag (RCCSettings.NGearCode))
				NGear = false;

			if(!automaticGear){

				if (RCC_InputMainManager.GetKeyDownFlag (RCCSettings.shiftGearUpCode))
					GearShiftUp ();

				if(RCC_InputMainManager.GetKeyDownFlag(RCCSettings.shiftGearDownCode))
					GearShiftDown();	
		...
		case RCC_SettingsData.ControllerType.XBox360One:
			...
			if (!string.IsNullOrEmpty (RCCSettingsInstance.Xbox_lowBeamHeadlightsKBValue)) {

				if (RCC_InputMainManager.GetButtonDownFlag (RCCSettings.Xbox_lowBeamHeadlightsKBValue))
					lowBeamHeadLightsOn = !lowBeamHeadLightsOn;

			}

			if (!string.IsNullOrEmpty (RCCSettingsInstance.Xbox_highBeamHeadlightsKBValue)) {

				if (RCC_InputMainManager.GetButtonDownFlag (RCCSettings.Xbox_highBeamHeadlightsKBValue))
					highBeamHeadLightsOn = true;
				else if (RCC_InputMainManager.GetButtonUpFlag (RCCSettings.Xbox_highBeamHeadlightsKBValue))
					highBeamHeadLightsOn = false;

			}

			if (!string.IsNullOrEmpty (RCCSettingsInstance.Xbox_startEngineKBValue) && RCC_InputMainManager.GetButtonDownFlag (RCCSettings.Xbox_startEngineKBValue))
				KillOrStartEngine ();

			if (!string.IsNullOrEmpty (RCCSettingsInstance.Xbox_trailerAttachDetachValue) && RCC_InputMainManager.GetButtonDownFlag (RCCSettings.Xbox_trailerAttachDetachValue))
				DetachTrailer ();

			float indicator = 0f;

			if (!string.IsNullOrEmpty (RCCSettingsInstance.Xbox_indicatorKBValue))
				indicator = Input.GetAxis (RCCSettings.Xbox_indicatorKBValue);

			float indicatorHazard = 0f;

			if (!string.IsNullOrEmpty (RCCSettingsInstance.Xbox_hazardIndicatorKBValue))
				Input.GetAxis (RCCSettings.Xbox_hazardIndicatorKBValue);
			...
			if (!automaticGear) {
				...
				if (!string.IsNullOrEmpty (RCCSettingsInstance.Xbox_shiftGearUpValue) && RCC_InputMainManager.GetButtonDownFlag (RCCSettings.Xbox_shiftGearUpValue))
					GearShiftUp ();

				if (!string.IsNullOrEmpty (RCCSettingsInstance.Xbox_shiftGearDownValue) && RCC_InputMainManager.GetButtonDownFlag (RCCSettings.Xbox_shiftGearDownValue))
					GearShiftDown ();	
		...
		case RCC_SettingsData.ControllerType.PS4:
			...
			if (!string.IsNullOrEmpty (RCCSettingsInstance.PS4_lowBeamHeadlightsKBValue)) {

				if (RCC_InputMainManager.GetButtonDownFlag (RCCSettings.PS4_lowBeamHeadlightsKBValue))
					lowBeamHeadLightsOn = !lowBeamHeadLightsOn;

			}

			if (!string.IsNullOrEmpty (RCCSettingsInstance.PS4_highBeamHeadlightsKBValue)) {

				if (RCC_InputMainManager.GetButtonDownFlag (RCCSettings.PS4_highBeamHeadlightsKBValue))
					highBeamHeadLightsOn = true;
				else if (RCC_InputMainManager.GetButtonUpFlag (RCCSettings.PS4_highBeamHeadlightsKBValue))
					highBeamHeadLightsOn = false;

			}

			if (!string.IsNullOrEmpty (RCCSettingsInstance.PS4_startEngineKBValue) && RCC_InputMainManager.GetButtonDownFlag (RCCSettings.PS4_startEngineKBValue))
				KillOrStartEngine ();

			if (!string.IsNullOrEmpty (RCCSettingsInstance.PS4_trailerAttachDetachValue) && RCC_InputMainManager.GetButtonDownFlag (RCCSettings.PS4_trailerAttachDetachValue))
				DetachTrailer ();

			float indicatorPS4 = 0f;

			if (!string.IsNullOrEmpty (RCCSettingsInstance.PS4_indicatorKBValue))
				indicatorPS4 = Input.GetAxis (RCCSettings.PS4_indicatorKBValue);
			
			float indicatorHazardPS4 = 0f;

			if (!string.IsNullOrEmpty (RCCSettingsInstance.PS4_hazardIndicatorKBValue))
				indicatorHazardPS4 = Input.GetAxis(RCCSettings.PS4_hazardIndicatorKBValue);
			...
			if (!automaticGear) {
				...
				if (!string.IsNullOrEmpty (RCCSettingsInstance.PS4_shiftGearUpValue) && RCC_InputMainManager.GetButtonDownFlag(RCCSettings.PS4_shiftGearUpValue))
					GearShiftUp();

				if (!string.IsNullOrEmpty (RCCSettingsInstance.PS4_shiftGearDownValue) && RCC_InputMainManager.GetButtonDownFlag(RCCSettings.PS4_shiftGearDownValue))
					GearShiftDown();
	...
	void FixedUpdate (){
		...
		for (int i = 0; i < allWheelColliders.Length; i++) {
			...
			if (allWheelColliders [i].canPowerFlag)
		...
		if(ESP)
			ESPCheck(FrontLeftWheelCollider.wheelColliderObject.steerAngle);

		if(RCCSettings.selectedBehaviorTypeR != null && RCCSettings.selectedBehaviorTypeR.applyRelativeTorqueFlag){
	...
	private void Engine (){
		...
		for (int i = 0; i < allWheelColliders.Length; i++) {
			...
			if (allWheelColliders [i].canPowerFlag)
				wheelRPM += allWheelColliders [i].wheelColliderObject.rpm;
	...
	private void Audio(){
		...
		windSound.volume = Mathf.Lerp (0f, RCCSettings.maxWindSoundVolumeValue, speed / 300f);
		windSound.pitch = UnityEngine.Random.Range(.9f, 1f);
		
		if(direction == 1)
			brakeSound.volume = Mathf.Lerp (0f, RCCSettings.maxBrakeSoundVolumeValue, Mathf.Clamp01((FrontLeftWheelCollider.wheelColliderObject.brakeTorque + FrontRightWheelCollider.wheelColliderObject.brakeTorque) / (brakeTorque * 2f)) * Mathf.Lerp(0f, 1f, FrontLeftWheelCollider.wheelColliderObject.rpm / 50f));
		else
	...
	private void ESPCheck(float steering){
		...
		frontSlip = FrontLeftWheelCollider.wheelHitData.sidewaysSlip + FrontRightWheelCollider.wheelHitData.sidewaysSlip;

		rearSlip = RearLeftWheelCollider.wheelHitData.sidewaysSlip + RearRightWheelCollider.wheelHitData.sidewaysSlip;
	...
	private void EngineSounds(){
		...
		case RCC_CarMainControllerV3.AudioType.OneSource:
		...
		case RCC_CarMainControllerV3.AudioType.TwoSource:
		...
		case RCC_CarMainControllerV3.AudioType.ThreeSource:
	...
	private void Wheels(){
		...
		for (int i = 0; i < allWheelColliders.Length; i++) {
			...
			if (allWheelColliders [i].canPowerFlag)
				allWheelColliders [i].ApplyMotorTorqueValue ((direction * allWheelColliders[i].powerMultiplierValue * (1f - clutchInput) * throttleInput * (1f + boostInput) * (engineTorqueCurve.Evaluate(engineRPM) * gears[currentGear].maxRatio * finalRatio)) / Mathf.Clamp(poweredWheels, 1, Mathf.Infinity));

			if (allWheelColliders [i].canSteerFlag)
				allWheelColliders [i].ApplySteeringInput (steerInput * allWheelColliders[i].steeringMultiplierValue, steerAngle);
			...
			if (!appliedBrake && handbrakeInput > .5f) {
				...
				if (allWheelColliders [i].canHandbrakeFlag)
					allWheelColliders [i].ApplyBrakeTorqueValue ((brakeTorque * handbrakeInput) * allWheelColliders [i].handbrakeMultiplierValue);
			...
			if (!appliedBrake && brakeInput >= .05f) {
				...
				if (allWheelColliders [i].canBrakeFlag)
					allWheelColliders [i].ApplyBrakeTorqueValue ((brakeInput * brakeTorque) * allWheelColliders [i].brakingMultiplierValue);
			...
			if(!appliedBrake)
				allWheelColliders [i].ApplyBrakeTorqueValue (0f);

			//	Checking all wheels. If one of them is not powered, reset.
			if (!allWheelColliders [i].canPowerFlag)
				allWheelColliders [i].ApplyMotorTorqueValue (0f);
			if (!allWheelColliders [i].canBrakeFlag)
				allWheelColliders [i].ApplyBrakeTorqueValue (0f);
			if (!allWheelColliders [i].canSteerFlag)
				allWheelColliders [i].ApplySteeringInput (0f, 0f);
	...
	private void AntiRollBars (){
		...
		bool groundedFL = FrontLeftWheelCollider.isGroundedFlag;
		
		if (groundedFL)
			travelFL = (-FrontLeftWheelCollider.transform.InverseTransformPoint(FrontLeftWheelCollider.wheelHitData.point).y - FrontLeftWheelCollider.wheelColliderObject.radius) / FrontLeftWheelCollider.wheelColliderObject.suspensionDistance;
		
		bool groundedFR = FrontRightWheelCollider.isGroundedFlag;
		
		if (groundedFR)
			travelFR = (-FrontRightWheelCollider.transform.InverseTransformPoint(FrontRightWheelCollider.wheelHitData.point).y - FrontRightWheelCollider.wheelColliderObject.radius) / FrontRightWheelCollider.wheelColliderObject.suspensionDistance;
		...
		bool groundedRL = RearLeftWheelCollider.isGroundedFlag;
		
		if (groundedRL)
			travelRL = (-RearLeftWheelCollider.transform.InverseTransformPoint(RearLeftWheelCollider.wheelHitData.point).y - RearLeftWheelCollider.wheelColliderObject.radius) / RearLeftWheelCollider.wheelColliderObject.suspensionDistance;
		
		bool groundedRR = RearRightWheelCollider.isGroundedFlag;
		
		if (groundedRR)
			travelRR = (-RearRightWheelCollider.transform.InverseTransformPoint(RearRightWheelCollider.wheelHitData.point).y - RearRightWheelCollider.wheelColliderObject.radius) / RearRightWheelCollider.wheelColliderObject.suspensionDistance;
	...
	public void CheckGrounded(){
		...
		for (int i = 0; i < allWheelColliders.Length; i++) {
			...
			if (!allWheelColliders [i].wheelColliderObject.isGrounded)
				grounded = false;
	...
	private void SteerHelper(){
		...
		for (int i = 0; i < allWheelColliders.Length; i++){
			...
			if (allWheelColliders[i].wheelHitData.normal == Vector3.zero)
				return;
		...
		steeringDirection.localRotation = Quaternion.Euler (0f, FrontLeftWheelCollider.wheelColliderObject.steerAngle, 0f);
	...
	private void TractionHelper(){
		...
		if (angle * FrontLeftWheelCollider.wheelColliderObject.steerAngle < 0) {
			FrontLeftWheelCollider.tractionHelpedSidewaysStiffnessValue = (1f - Mathf.Clamp01 (tractionHelperStrength * Mathf.Abs (angularVelo)));
		} else {
			FrontLeftWheelCollider.tractionHelpedSidewaysStiffnessValue = 1f;
		}

		if (angle * FrontRightWheelCollider.wheelColliderObject.steerAngle < 0) {
			FrontRightWheelCollider.tractionHelpedSidewaysStiffnessValue = (1f - Mathf.Clamp01 (tractionHelperStrength * Mathf.Abs (angularVelo)));
		} else {
			FrontRightWheelCollider.tractionHelpedSidewaysStiffnessValue = 1f;
		}
	...
	public IEnumerator ChangeGear(int gear){
		...
		if(RCCSettings.useTelemetryObject)
			print ("Shifted to: " + (gear).ToString()); 

		if(gearShiftingClips.Length > 0){
			
			gearShiftingSound = CreateNewAudioSource(RCCSettings.audioMixerValue, gameObject, gearSoundPosition, "Gear Shifting AudioSource", 1f, 5f, RCCSettings.maxGearShiftingSoundVolumeValue, gearShiftingClips[UnityEngine.Random.Range(0, gearShiftingClips.Length)], false, true, true);
	...
	private void NOS(){
		...
		if(!NOSSound)
			NOSSound = CreateNewAudioSource(RCCSettings.audioMixerValue, gameObject, exhaustSoundPosition, "NOS Sound AudioSource", 5f, 10f, .5f, NOSClip, true, false, false);
	...
	private void Turbo(){
		...
		if (!turboSound) {
			...
			turboSound = CreateNewAudioSource (RCCSettings.audioMixerValue, gameObject, turboSoundPosition, "Turbo Sound AudioSource", .1f, .5f, 0, turboClip, true, true, false);
			AddNewHighPassFilter (turboSound, 10000f, 10);
		...
		if(turboBoost >= 25f){
			...
			if(turboBoost < (turboSound.volume * 30f)){
				...
				if(!blowSound.isPlaying){
					...
					blowSound.clip = RCCSettings.blowoutClipMass[UnityEngine.Random.Range(0, RCCSettings.blowoutClipMass.Length)];
					blowSound.Play();
	...
	private void DriftVariables(){
		...
		if(Mathf.Abs(RearRightWheelCollider.wheelHitData.sidewaysSlip) > .25f)
			driftingNow = true;
		else
			driftingNow = false;
		
		if(speed > 10f)
			driftAngle = RearRightWheelCollider.wheelHitData.sidewaysSlip * .75f;
		else
			driftAngle = 0f;
			...
			if (colRelVel.magnitude * cos >= minimumCollisionForce) {
				...
				if (useWheelDamage){
					...
					for (int i = 0; i < allWheelColliders.Length; i++){
						...
						if (point.magnitude <= wheelDamageRadius){
							...
							allWheelColliders[i].damagedToeValue += (UnityEngine.Random.Range(-(colRelVel.magnitude * cos), (colRelVel.magnitude * cos))) * wheelDamageMultiplier;
							allWheelColliders[i].damagedCamberValue += (UnityEngine.Random.Range(-(colRelVel.magnitude * cos), (colRelVel.magnitude * cos))) * wheelDamageMultiplier;
							allWheelColliders[i].damagedCasterValue += (UnityEngine.Random.Range(-(colRelVel.magnitude * cos), (colRelVel.magnitude * cos))) * wheelDamageMultiplier;
				...
				if (crashClips.Length > 0){
					...
					if (collision.contacts[0].thisCollider.gameObject.transform != transform.parent){
						...
						crashSound = CreateNewAudioSource(RCCSettings.audioMixerValue, gameObject, "Crash Sound AudioSource", 5, 20, RCCSettings.maxCrashSoundVolumeValue, crashClips[UnityEngine.Random.Range(0, crashClips.Length)], false, true, true);
	...
	void OnDrawGizmos(){
		...
		if(Application.isPlaying){
			...
			for(int i = 0; i < allWheelColliders.Length; i++){
				...
				allWheelColliders[i].wheelColliderObject.GetGroundHit(out hit);
	...
	public void DetachTrailer(){
		...
		attachedTrailer.DetachTrailerCamera ();
	...
	void OnDestroy(){
		...
		if(canControl){
			...
			if(gameObject.GetComponentInChildren<RCC_CameraController>())
				gameObject.GetComponentInChildren<RCC_CameraController>().transform.SetParent(null);
