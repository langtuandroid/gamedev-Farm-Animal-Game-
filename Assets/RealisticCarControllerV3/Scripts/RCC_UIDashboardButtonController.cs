...
public class RCC_UIDashboardButtonController : MonoBehaviour, IPointerClickHandler {
	
	[FormerlySerializedAs("_buttonType")] public ButtonType _buttonTypeR;
	...
	private Scrollbar gearSliderScrollbar;

	[FormerlySerializedAs("gearDirection")] public int gearDirectionValue = 0;
	...
	private void Start(){
		...
		if(_buttonTypeR == ButtonType.Gear && GetComponentInChildren<Scrollbar>()){
			
			gearSliderScrollbar = GetComponentInChildren<Scrollbar>();
			gearSliderScrollbar.onValueChanged.AddListener (delegate {ChangeNGear ();});
	...
	private void OnClickedAction () {
		...
		switch (_buttonTypeR) {
			...
			case ButtonType.Neutral:
				...
				RCC_Manager.StopRecordLastReplay();

				break;

			case ButtonType.ChangeCamera:

				RCC_Manager.ChangeCameraMode();
		...
		case ButtonType.LeftIndicator:
				...
				if (RCC_SceneManager.Instance.activePlayerVehicle) {
					...
					if (RCC_SceneManager.Instance.activePlayerVehicle.indicatorsOn != RCC_CarMainControllerV3.IndicatorsOn.Left)
						RCC_SceneManager.Instance.activePlayerVehicle.indicatorsOn = RCC_CarMainControllerV3.IndicatorsOn.Left;
					else
						RCC_SceneManager.Instance.activePlayerVehicle.indicatorsOn = RCC_CarMainControllerV3.IndicatorsOn.Off;
		...
		case ButtonType.RightIndicator:
				...
				if (RCC_SceneManager.Instance.activePlayerVehicle) {
					...
					if (RCC_SceneManager.Instance.activePlayerVehicle.indicatorsOn != RCC_CarMainControllerV3.IndicatorsOn.Right)
						RCC_SceneManager.Instance.activePlayerVehicle.indicatorsOn = RCC_CarMainControllerV3.IndicatorsOn.Right;
					else
						RCC_SceneManager.Instance.activePlayerVehicle.indicatorsOn = RCC_CarMainControllerV3.IndicatorsOn.Off;
		...
		case ButtonType.HazardLights:
				...
				if (RCC_SceneManager.Instance.activePlayerVehicle) {
					...
					if (RCC_SceneManager.Instance.activePlayerVehicle.indicatorsOn != RCC_CarMainControllerV3.IndicatorsOn.All)
						RCC_SceneManager.Instance.activePlayerVehicle.indicatorsOn = RCC_CarMainControllerV3.IndicatorsOn.All;
					else
						RCC_SceneManager.Instance.activePlayerVehicle.indicatorsOn = RCC_CarMainControllerV3.IndicatorsOn.Off;
	...
	public void ChangeNGear(){

		if (!RCC_SceneManager.Instance.activePlayerVehicle)
			return;

		if(gearDirectionValue == Mathf.CeilToInt(gearSliderScrollbar.value * 2))
			return;

		gearDirectionValue = Mathf.CeilToInt(gearSliderScrollbar.value * 2);

		RCC_SceneManager.Instance.activePlayerVehicle.semiAutomaticGear = true;

		switch(gearDirectionValue){
